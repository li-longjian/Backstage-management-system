    ...
    <el-dialog
      ...
      <div class="form">
        <el-form :model="form_data" status-icon :rules="form_rules" ref="registerForm" label-width="80px"
          ...
          <el-form-item v-if="dialog.option=== 'add' " label="密码" prop="password" class="el-form-item">
      ...
      submitForm(form) {
        this.$refs[form].validate(valid => {
          if (valid) {
            ...
            //设置管理员
            if (this.form_data.identity === '系统管理员') {
              this.form_data.isAdmin = true
            }
            if (option === 'add') {
              this.$axios.post('/register', this.form_data).then(res => {
            ...
            //处理编辑功能：
            if (option === 'edit'){
              this.$axios.post(`/setuser/${this.form_data.id}`,this.form_data).then(res =>{
                this.$message({
                  message:'修改成功',
                  type: 'success',
                  center: true
